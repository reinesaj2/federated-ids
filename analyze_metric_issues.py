    ...
    try:
        ...
        if "l2_to_benign_mean" in df.columns:
            ...
            if len(l2_col) > 0:
                ...
                if l2_zeros > 1:
                    results["l2_issues"].append(f"Multiple L2=0 values (possible bug)")
...
def main():
    ...
    for csv_file in runs_dir.rglob("metrics.csv"):
        ...
        has_issues = (len(results.get("cosine_issues", [])) > 0 or 
                     len(results.get("l2_issues", [])) > 0 or 
                     len(results.get("f1_issues", [])) > 0)
