...
def plot_server_metrics(metrics_path: str, output_path: str):
    ...
    if time_series is not None and not time_series.isna().all():
        axes[0, 0].plot(df['round'], time_series, 'o-', color='blue', label='Aggregation Time (ms)')
    ...
    # Plot 3: Update norms
    if 'update_norm_mean' in df.columns and not pd.to_numeric(df['update_norm_mean'], errors="coerce").isna().all():
...
def plot_client_metrics(client_metrics_paths: list, output_path: str):
    ...
    plotted_loss = plotted_acc = plotted_norm = plotted_overlay = False
    ...
    for i, client_path in enumerate(client_metrics_paths):
        ...
        if {'macro_f1_argmax', 'f1_bin_tau'}.issubset(df.columns):
            ...
            axes[1, 1].plot(rounds, series_a, 'o-', color=color, alpha=0.7, label=f'Argmax F1 C{client_id}')
            axes[1, 1].plot(rounds, series_b, 'x--', color=color, alpha=0.7, label=f'Bin@τ F1 C{client_id}')
            plotted_overlay = True
        elif acc_series is not None and not acc_series.isna().all():
            axes[1, 1].plot(rounds, acc_series, 'o-', color=color, alpha=0.7, label=f'Acc C{client_id}')
            plotted_overlay = True
