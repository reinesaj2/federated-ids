...
def parse_config(name):
    config = {}
    if "comp_fedavg" in name: config["aggregation"] = "FedAvg"
    elif "comp_krum" in name: config["aggregation"] = "Krum"
    elif "comp_bulyan" in name: config["aggregation"] = "Bulyan"
    elif "comp_median" in name: config["aggregation"] = "Median"
...
for run_path in run_paths[:5]:
    ...
    if s_path.exists():
        ...
        for k,v in config.items(): sdf[k] = v
    ...
    if c_files:
        ...
        for k,v in config.items(): cdf[k] = v
