#!/usr/bin/env python3
"""
Run Objective 2 robust aggregation experiments sequentially - FIXED VERSION.
Uses direct ExperimentConfig instead of broken dimension parameter.
"""
import json
import subprocess
import sys
...
QUEUE_FILE = Path("obj2_robust_agg_queue_fixed.json")
PROGRESS_FILE = Path("obj2_robust_agg_progress_fixed.json")
LOG_FILE = Path("obj2_robust_agg_fixed.log")
...
def run_single_experiment(exp: dict) -> bool:
    """Run a single robust aggregation experiment using aggregation dimension."""
    agg = exp["aggregation"]
    alpha = exp["alpha"]
    seed = exp["seed"]
    ...
    # Use --dimension aggregation which properly respects --aggregation-methods
    # and override --alpha-values to use our specific alpha
    cmd = [
        "python",
...
def main():
    ...
    log_message("=" * 80)
    log_message("OBJECTIVE 2 ROBUST AGGREGATION QUEUE (FIXED)")
    log_message("Using --dimension aggregation to properly respect --aggregation-methods")
    log_message("=" * 80)
