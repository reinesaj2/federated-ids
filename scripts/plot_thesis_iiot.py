...
def plot_utility_comparison(data: pd.DataFrame, output_dir: Path, style: ThesisStyle):
    ...
    ax.text(0.5, 0.5, "FedAvg F1 ~ 0.0 (Catastrophic Failure)", transform=ax.transAxes, fontsize=10, color='red', ha='center')
...
def plot_fedprox_efficacy(data: pd.DataFrame, output_dir: Path, style: ThesisStyle):
    ...
    try:
        sns.lineplot(
            data=plot_data, x="round", y="macro_f1_global", hue="aggregation", style="aggregation", markers=True, ax=ax, errorbar=("ci", 95)
        )
    except:
        sns.lineplot(data=plot_data, x="round", y="macro_f1_global", hue="aggregation", style="aggregation", markers=True, ax=ax, ci=95)
...
def plot_system_overhead(data: pd.DataFrame, output_dir: Path, style: ThesisStyle):
    ...
    try:
        ...
        sns.lineplot(data=data, x="round", y=metric, hue="aggregation", style="aggregation", markers=True, dashes=linestyles, ax=ax, ci=95)
...
def main():
    ...
    if client_df is not None:
        ...
        try:
            ...
            client_agg = client_agg.rename(columns={"macro_f1_before": "macro_f1_global", "macro_f1_after": "macro_f1_personalized"})
    ...
    target_alpha = "0.5"
    target_adv = 30
    robust_df = server_df[(server_df["alpha"].astype(str) == target_alpha) & (server_df["adversary"] == target_adv)].copy()

    robust_client_df = None
    if client_df is not None:
        robust_client_df = client_df[(client_df["alpha"].astype(str) == target_alpha) & (client_df["adversary"] == target_adv)].copy()
    ...
    if robust_client_df is not None and not robust_client_df.empty:
        ...
        robust_client_df = robust_client_df.rename(
            columns={"macro_f1_before": "macro_f1_global", "macro_f1_after": "macro_f1_personalized"}
        )
