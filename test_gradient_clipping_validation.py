...
def test_legitimate_clients_not_clipped():
    ...
    # Mock the fit method to capture gradient clipping behavior
    original_fit = client.fit
    
    def mock_fit(parameters, config):
        # Set parameters
        for param, new_param in zip(client.model.parameters(), parameters):
...
def test_clipping_factor_configurable():
    ...
    for clip_factor in [1.0, 2.0, 5.0]:
        client = _make_test_client(
            adversary_mode="grad_ascent", 
            adversary_clip_factor=clip_factor
        )
        ...
        def mock_clip_grad_norm(parameters, max_norm, norm_type=2.0):
            client._clipping_calls.append(max_norm)
            return original_clip_grad_norm(parameters, max_norm, norm_type)
        ...
        assert all(call == clip_factor for call in client._clipping_calls), \
