...
class TorchClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        if secure_agg_enabled:
            self.secret_shares = [generate_secret_shares(w.shape, seed=hash(self.round_num) & 0x7FFFFFFF) for w in weights_after]
            weights_after = [mask_updates(w, share) for w, share in zip(weights_after, self.secret_shares)]
